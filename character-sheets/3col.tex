...
\newdimen\XCoord
\newdimen\YCoord
\newcommand*{\ExtractCoordinates}[1]{%
...
\newcommand\setdeltay[3]{%
  \ExtractCoordinates{#2}%
  #1=\YCoord
  \ExtractCoordinates{#3}%
  \advance#1 by -\YCoord
}
...
%%%%    
%%%%    %% \tikzpoint{<anything TikZ would accept inside (...)>}
%%%%    %% Expands to a \pgfpoint{<x>}{<y>}
%%%%    %\newcommand*\tikzpoint[1]{%
%%%%    %  \begingroup
%%%%    %  \tikz@scan@one@point\tikzpoint@aux{(#1)}%
%%%%    %}
%%%%    %\def\tikzpoint@aux#1{%
%%%%    %  \endgroup
%%%%    %  #1% <- this is the \pgfpoint returned by TikZ’s parser
%%%%    %}
%%%%    %\makeatother
%%%%    
%%%%    
    \renewcommand\setdeltay[3]{%
      \pgfpointtikz{#2}%
      #1=\pgf@y
      \pgfpointtikz{#3}%
      \advance#1 by -\pgf@y
...
\draw [fill=red] (motivation.label) circle [radius=4pt];
...
\node[narrowcolumnbox,% width=38.002mm,
      fill=white,below=4mm of inspiration,align=center,
      height=35mm,
    ]
    (saving throws)
...
\path (proficiency bonus.south) ++ (0,-5mm) coordinate (prof top fit);
  ...
  \setdeltay\sectionheight{prof top fit}{prof bot fit}
  \node (proficiencies) [narrowcolumnbox,fill=proficiencies,height=\sectionheight,
        anchor=north east,at=(prof top fit -| saving throws.east),
                        ] % at (prof bot fit -| saving throws.west)
...
\setdeltay\sectionheight{attacks upper left fit}{attacks lower right fit}

  \node (attacks) [columnbox,fill=attacks,height=\sectionheight,
                   anchor=north west,at=(attacks upper left fit)]
   {
