...
\newsectionenv[width=\colwidth,decorated labeled clipped rectangle]{equipment}{EQUIPMENT}
   {%
     \medskip
     \noindent
     \hspace*{21mm}%
...
\newsectionenv[width=38mm,labeled rectangle]{proficiencies}{PROFICIENCIES}
   {\par\medskip
...
\else
  \newdimen\profheight
  \setdeltay\profheight{prof top fit}{prof bot fit}
  \node (proficiencies) [proficiencies,decorated labeled stub rectangle,draw,width=38.002mm,height=\profheight,
        anchor=north east,at=(prof top fit -| saving throws.east),
inner sep=10pt,
                        ] % at (prof bot fit -| saving throws.west)
   {\ifDNDdefined{PROFICIENCIES}{%
     \begin{minipage}[t]{38mm-20pt}
     \long\def\split#1\profskip#2\nil{%
       \def\profbefore{#1}% before part
       \def\profafter{#2}%  after part (empty if no \profskip in input)
     }
...
\end{proflist}
  ...
  \nodepart{label}\textsc{Proficiencies}
   }
   ;
\fi
