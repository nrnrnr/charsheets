...
This document provides a comprehensive alphabetical reference for all YAML keys used in the D&D character sheet system. Keys are categorized as **User Keys** (intended for direct use in YAML files) or **System Keys** (generated automatically by the system).
...
**Type**: List of structured objects  
**Required**: No  
**Template Usage**: Not displayed in current template  
...
**Issue**: `MAGIC FONT` key found in YAML files but not declared in charsheet.sty  
**Recommendation**: Add `\newDND{MAGIC FONT}` declaration or standardize font control mechanism
...
- **CLASS**: Extracted from "CLASS & LEVEL"
- **LEVEL**: Extracted from "CLASS & LEVEL"  
